...
function combat_sequence(char1,char2){
	...
	if (char2.hp <= 0){ //opponent is defeated
...
$(".character").on("click",function(){
	if ($(".character","#fighter1").length == 0){ //fighter 1 empty
		$("#fighter1").append($(this));		
	}
	else if  (     ($(".character","#fighter2").length == 0)   &&   ($(this).parents("#fighter1").length == 0)  ){	//fighter 2 isemtpy and yourself not in fighter 1
...
function character(check){
		...
		else if (check === "hagrid"){
			check = hagrid
		}
		else if (check === "dumbledore"){
			check = dumbledore
		}
		else if (check === "voldemort"){
			check = voldemort
		}
		else{
		}
